...
// CLI entry point — allows `node unified-activation-pipeline.js <agentId>`
if (require.main === module) {
  const agentId = process.argv[2];
  if (!agentId || agentId === '--help') {
    console.log(`Usage: node unified-activation-pipeline.js <agentId>`);
    console.log(`Valid agents: ${ALL_AGENT_IDS.join(', ')}`);
  ...
  UnifiedActivationPipeline.activate(agentId)
    ...
    .catch(err => {
      console.error(`[UAP] Activation failed: ${err.message}`);
      process.exit(1);
    });
}
